...
def evaluate(model, loader , number_cluster = 1):
    model.eval()
    running_loss = 0
    running_acc = 0
    count = 0
    ...
    for i, (bx, by) in enumerate(loader):
        ...
        pred = (pred/number_cluster).type('torch.IntTensor').cuda()
        pred = pred.type('torch.cuda.LongTensor')
        ########

        running_acc += (pred == by).float().sum(0).item()
        count += by.shape[0]
...
def load_model(dataset_name, num_classes, drop_rate=0.3, use_vgg_countermeasure=False, training_mode = "cross-entropy" , arch ='wrn' ):
    ...
    if dataset_name in ['cifar10', 'cifar100']:
        if use_vgg_countermeasure:
            ...
            # if (training_mode == "cross-entropy"):
...
def load_data(dataset_name, train=True, deterministic=False, seed=1, training_mode = "cross-entropy", custom = False , data_path = '/kaggle/working/train_vic'):
    if train == False:
    ...
    if dataset_name in ['cub200']:
        ...
        if train and not deterministic:
            ...
            if training_mode == "contrastive":
                transform = DuplicateSampleTransform(transform)
        else:
            transform = transforms.Compose([
        ...
        if custom:
            train_set_data = torch.load(data_path)
            dataset = CustomDataset(train_set_data, transform=transform)
        ...
        if custom:
            ...
            dataset = datasets.ImageFolder(f'/kaggle/working/caltech256/caltech256/{split}',
                                      transform=transform)
        ...
        if custom:
            ...
            dataset = datasets.ImageFolder(f'/kaggle/working/ImageNet_CUB200/ImageNet_CUB200/{split}', transform=transform)
        ...
        if dataset_name in ['imagenet_cifar10']:
            ...
            if custom:
                ...
                dataset = datasets.ImageFolder(f'/kaggle/working/ImageNet_CIFAR10/ImageNet_CIFAR10/{split}', transform=transform)
            ...
            if custom:
                ...
                dataset = datasets.ImageFolder(f'/kaggle/working/ImageNet_CIFAR100/ImageNet_CIFAR100/{split}', transform=transform)
