...
def main(args):
    ...
    teacher = load_model(args.eval_data, num_classes, arch=args.arch)
    teacher_path = '/kaggle/working/save_path/model'.format(args.eval_data.split('_')[0])
    assert os.path.exists(teacher_path), 'Expected model in teacher path: {}'.format(teacher_path)
    teacher.load_state_dict(torch.load('/kaggle/working/save_path/model'))
    teacher.eval()
    ...
    if args.defense == 'MAD':
        surrogate = load_model(args.eval_data, num_classes, arch=args.arch)
        surr_root = '/kaggle/working/save_path/model-surrogate'
        surrogate.load_state_dict(torch.load('/kaggle/working/save_path/model-surrogate'))
        surrogate.eval()
        ...
        if args.defense in ['ALL-ONES', 'ALL-ONES_focused', 'ALL-ONES_0', 'ALL-ONES_10', 'ALL-ONES_20', 'ALL-ONES_30', 'ALL-ONES_40',
            ...
            misinformation_model = load_model(args.eval_data, num_classes, arch=args.arch)
        ...
        if args.eval_data in ['cifar10', 'cifar100']:
            oe_model = load_model(args.eval_data, num_classes, arch=args.arch).module
